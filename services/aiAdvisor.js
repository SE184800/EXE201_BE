...
async function answerWithAI({message,history=[],calendar,apiKey,model,fetcher=fetch}) {
 ...
 const rows=calendar.items.slice(0,100).map(({itemId,name,unit,quantity,purchasePrice,expiryDate,averageDailySales,method,recommendation,weekendRatio,isDemo,calendarNote})=>({itemId,name,unit,quantity,purchasePrice,expiryDate,averageDailySales,method,recommendation,weekendRatio,isDemo,calendarNote}));
 try {
